...
def evaluate_fusion(args):
    ...
    # Use directories directly without checking for test subfolder
    spatial_test_dir = args.spatial_dir
    freq_test_dir = args.freq_dir

    print(f"[INFO] Spatial test dir: {spatial_test_dir}")
    print(f"[INFO] Frequency test dir: {freq_test_dir}")

    if not os.path.exists(spatial_test_dir) or not os.path.exists(freq_test_dir):
    ...
    test_ds = DualImageDataset(spatial_test_dir, freq_test_dir, transform=spatial_tf, spec_size=(128, 128))
    test_loader = DataLoader(test_ds, batch_size=32, shuffle=False, num_workers=0)

    class_names = test_ds.spatial_dataset.classes
...
if __name__ == "__main__":
    ...
    parser.add_argument("--spatial_dir", type=str, default="./Images", help="Path to spatial image dataset")
    parser.add_argument("--freq_dir", type=str, default="./Frequency_Spectrums", help="Path to frequency spectrum dataset")
